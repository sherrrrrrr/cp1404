...
def display_welcome_message(movies):
    print("Movies2See 1.0 - by JiaxinLi")
    watched_count = sum(1 for movie in movies if movie[3] == WATCHED)
    print(f"{len(movies)} movies loaded, {watched_count} movies watched")
...
def add_movie(movies):
    ...
    if not title or not year.isdigit() or not category:
        print("Input cannot be blank, and year must be a number")
    else:
